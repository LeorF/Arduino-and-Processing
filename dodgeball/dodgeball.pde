...
void setup() {
    ...
    for (int i=0; i<5; i++) { //Creates 5 balls
      balls[i] = new ball();
    }
    for (int j = 0; j<5; j++) { //Sets random starting locations for the balls
      balls[j].setXloc();
      balls[j].setYloc();
      balls[j].setXdir();
      balls[j].setYdir();
...
void draw(){
    ...
    if (gameLose == false){
      for (int i=0; i<5; i++){
        ...
        else { //If the player did not hit a ball, then the game continues playing
          cam();          
            for (int l=0; l<5; l++){
              balls[l].move();           //Uses the ball's method to move the ball
            }

            for (int j=0; j<5; j++){ //Draws the balls
              
              image(bomb, balls[j].xLocGet(), balls[j].yLocGet());

            }
            for (int h=0; h<5; h++){ //Bounces the balls off the walls
              balls[h].bounce(640, 480);
            }
          player1.setXLoc(mouseX);   //Gets the mouse coordinates, and ascribes it to the player object
          player1.setYLoc(mouseY);
          
          image (character, player1.xLocGet(), player1.yLocGet());
          
          
          image (health, healBall.xLocGet(),healBall.yLocGet());
        }
      }
      // print clock
    ...
    if (gameLose == true){ //When the game is lost, it goes and prints the game over screen
      background(0);
      fill(255);
      textAlign(CENTER);
      if (score > highScore) {
      ...
      text ("You Lived: " + holdSec + " Seconds \n Longest Time Lived: " + maxSec + " Seconds", 350, 450, 0);
      
    }
    
  }

  void keyPressed() { //If the person presses any key...
    secondsCounter = 0;
    ...
    for (int j = 0; j<5; j++) { //Because we're playing the game again, we need to reset the ball locations
      balls[j].setXloc();
      balls[j].setYloc();
      balls[j].setXdir();
      balls[j].setYdir();
...
void track() {
  ...
  if (worldRecord < 10) { 
    ...
    */
    player1.setXLoc(closestX);
    player1.setYLoc(closestY);
    //image(character, closestX, closestY);
  }
}
...
class healing {
  ...
  int healingGet() {
    ...
    //Methods to set various aspects of the ball object depending on what the main wants it to do
  double setLocX(){
    return xLoc = (int) (Math.random()*500+1);
  }
  double setLocY(){
    return yLoc = (int) (Math.random()*500+1);
  }
